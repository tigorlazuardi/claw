...
func (s *Claw) UpdateDevice(ctx context.Context, req *clawv1.UpdateDeviceRequest) (*clawv1.UpdateDeviceResponse, error) {
	...
	err = Devices.UPDATE(ColumnList(columns)).MODEL(model.Devices{
		ID:                    new(int64),
		Name:                  Defer(req.Name),
		Width:                 int64(Defer(req.Width)),
		Height:                int64(Defer(req.Height)),
		SaveDir:               Defer(req.SaveDir),
		FilenameTemplate:      Defer(req.FilenameTemplate),
		ImageMaxWidth:         int64(Defer(req.ImageMaxWidth)),
		ImageMinHeight:        int64(Defer(req.ImageMinHeight)),
		ImageMaxHeight:        int64(Defer(req.ImageMaxHeight)),
		AspectRatioDifference: Defer(req.AspectRatioDifference),
		NsfwMode:              int64(*req.Nsfw),
		ImageMinFileSize:      int64(Defer(req.ImageMinFilesize)),
		ImageMaxFileSize:      int64(Defer(req.ImageMaxFilesize)),
		ImageMinWidth:         int64(Defer(req.ImageMinWidth)),
	...
	device := &clawv1.Device{
		...
		ImageMinFilesize:      uint64(out.ImageMinFileSize),
		ImageMaxFilesize:      uint64(out.ImageMaxFileSize),
